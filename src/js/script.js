...
function renderWeather(data) {
  return (renderDisplay.innerHTML = `
          <div class="w-full">
            ...
            }" class="size-20 mx-auto lg:mx-6"  alt="${data.location.name}">
          </div>
          <div class="lg:flex block items-center text-center lg:w-[70%] w-full justify-between lg:gap-4 px-4 lg:ml-6 lg:absolute lg:bottom-10">
            <p class="lg:text-6xl text-xl font-semibold">${Math.round(
              data.current.temp_c
            )}&#8451;</p>
            <div>
              <h1 class="text-center font-medium lg:text-2xl">${
                data.location.name
              }, ${data.location.country}</h1>
              <p>${data.location.localtime}</p>
            </div>
            <div class="hidden lg:block">
              <h1 class="lg:px-8 lg:text-xl font-semibold">${
